...
def test_network():
    layers: list[network.Layer] = [network.DenseLayer(input_size=2, output_size=4, activation='relu'), network.DenseLayer(input_size=4, output_size=4, activation='sigmoid'), network.DenseLayer(input_size=4, output_size=2, activation='sigmoid'), network.Softmax()]
    ...
    y0 = network.np.array([[1, 0], [0, 1], [0, 1], [1, 0]])
...
def test_gru_layer():
    ...
    layers: list[network.Layer] = [GRULayer(input_size=3, hidden_size=4), network.DenseLayer(input_size=4, output_size=2, activation='sigmoid')]
    net = network.Network(layers)
    X_seq = network.np.array([[0.1, 0.2, 0.3], [0.2, 0.1, 0.0], [0.0, 0.1, 0.2], [0.1, 0.0, 0.1], [0.2, 0.3, 0.1]])
    y_seq = network.np.array([[1, 0]])
