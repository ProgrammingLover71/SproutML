...
def get_loss_derivative(name: loss_func):
	if name == 'mse':
...
def mean_squared_error_dr(predictions: np.ndarray, targets: np.ndarray) -> np.ndarray:
	"""Computes the derivative of the Mean Squared Error (MSE) loss function with respect to the predictions."""
